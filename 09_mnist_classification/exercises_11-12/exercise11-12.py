...
FIGS_WRITTEN = []
TABLES_PATH = SCRIPT_DIR / "terminal_tables.txt"
EVAL_BLOCKS = []
...
save_current_figure("mnist_classification_fig_001.png")
plt.show()
...
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
    filepath='best_mdl.keras', # set to 'best_mdl.hdf5' if you want only the best model, overall or to '{epoch:02d}-{val_loss:.5f}.hdf5' if you want to save the best model as the training proceed (best model over time)
...
save_current_figure("mnist_classification_fig_002.png")
plt.show()
...
EVAL_BLOCKS.append(
    "##########Training set\nConfusion matrix\n"
    + np.array2string(cmtx)
    + "\nAccuracy: "
    + str(np.mean(labels_train == y_pred))
)
...
EVAL_BLOCKS.append(
    ...
    + np.array2string(cmtx)
    + "\nAccuracy: "
    + str(np.mean(labels_valid == y_pred))
)
...
EVAL_BLOCKS.append(
    ...
    + np.array2string(cmtx)
    + "\nAccuracy: "
    + str(np.mean(labels_test == y_pred))
)
...
save_current_figure("mnist_classification_fig_003.png")
...
tables_content = "\n\n".join(EVAL_BLOCKS) + "\n\nPredicted class of my digit: " + str(y_pred_my_digit)
